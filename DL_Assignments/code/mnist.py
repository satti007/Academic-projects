...
train_data = pd.read_csv('../data/kaggle_mnsit_train.csv').as_matrix()
test_data  = pd.read_csv('../data/kaggle_mnsit_test.csv').as_matrix()
...
model_info = model.fit(X_train, Y_train,callbacks=[earlyStopping],batch_size=8, epochs=100,verbose=2,validation_split=0.15)
plot_model_history(model_history,'kaggle.png')
